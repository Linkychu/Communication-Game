...
public class Recursive : Maze
{
    ...
    {
        Vector2Int pos = new Vector2Int(SeededRandom.Range(1, size.x - 1), SeededRandom.Range(1, size.y - 1));
        Generate(pos);
    }
    ...
    public override void AddRooms(int count, int minSize, int maxSize)
    {
        ...
        {
            Vector2Int startPos = new Vector2Int(SeededRandom.Range(3, size.x - 3), SeededRandom.Range(3, size.y - 3));
            Vector2Int roomSize = new Vector2Int(SeededRandom.Range(minSize, maxSize), SeededRandom.Range(minSize, maxSize));
